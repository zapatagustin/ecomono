...
Loaded on demand — NOT part of the always-on CLAUDE.md. Read this in full before running any `/ecomono-sdd-*` command or coordinating multi-agent delegation. Moved out of CLAUDE.md to save ~7k input tokens/turn on non-SDD work; the protocol content is unchanged.
...
- `ecomono-memory` — default when available; persistent memory across sessions
- `openspec` — file-based artifacts; use only when user explicitly requests
- `hybrid` — both backends; cross-session recovery + local files; more tokens per op
- `none` — return results inline only; recommend enabling ecomono-memory or openspec

### Commands

Skills (appear in autocomplete):

- `/ecomono-sdd-init` → initialize SDD context; detects stack, bootstraps persistence
- `/ecomono-sdd-explore <topic>` → investigate an idea; reads codebase, compares approaches; no files created
- `/ecomono-sdd-status [change]` → read-only structured status for active change, artifacts, tasks, and next action
- `/ecomono-sdd-apply [change]` → implement tasks in batches; checks off items as it goes
- `/ecomono-sdd-verify [change]` → validate implementation against specs; reports CRITICAL / WARNING / SUGGESTION
- `/ecomono-sdd-archive [change]` → close a change and persist final state in the active artifact store
- `/ecomono-sdd-onboard` → guided end-to-end walkthrough of SDD using your real codebase

Meta-commands (type directly — orchestrator handles them, won't appear in autocomplete):

- `/ecomono-sdd-new <change>` → start a new change by delegating exploration + proposal to sub-agents
- `/ecomono-sdd-continue [change]` → run the next dependency-ready phase via sub-agent(s)
- `/ecomono-sdd-ff <name>` → fast-forward planning: proposal → specs → design → tasks

`/ecomono-sdd-new`, `/ecomono-sdd-continue`, and `/ecomono-sdd-ff` are meta-commands handled by YOU. Do NOT invoke them as skills.

### Native SDD Dispatcher Guard

Before routing, continuing, applying, verifying, or archiving an SDD change, **first determine this session's artifact store** from the cached Session Preflight / Artifact Store Mode choice. If the store is not yet established, resolve it before continuing — check `ecomono-sdd-init/{project}` in ecomono-memory and treat the change as `ecomono-memory`-backed when no OpenSpec store was selected. **Then scope the native dispatcher by artifact store.** The native dispatcher (`gentle-ai sdd-continue [change] --cwd <repo>` or `gentle-ai sdd-status [change] --cwd <repo> --json --instructions`) reads ONLY OpenSpec file artifacts under `openspec/changes/` and always emits `artifactStore: openspec`; it cannot observe ecomono-memory-backed changes. **When the session artifact store is `ecomono-memory`, do NOT invoke the dispatcher at all** — it is blind to the change and its `blocked`, `Active OpenSpec change not found`, or `nextRecommended: sdd-new` output is meaningless; resolve status entirely from ecomono-memory (`mem_search` + `mem_get_observation` on the change's topic keys such as `sdd/{change-name}/tasks`) using the manual status schema. Only when the session artifact store is `openspec` or `hybrid` should you run the dispatcher when `gentle-ai` is available and treat its native status JSON as authoritative over prompt inference. Route only by `nextRecommended` and dependency states; never infer from free text. If `blockedReasons` is non-empty, do not proceed to apply, archive, or terminal work. If `nextRecommended` is `verify`, verification/remediation may run only to refresh evidence; if `nextRecommended` is `resolve-blockers`, report `blockedReasons` and stop; if `nextRecommended` is a planning token (`propose`, `spec`, `design`, or `tasks`), launch the corresponding planning phase. If the binary is unavailable, fall back to the existing prompt contract and manual status schema.

### SDD Init Guard (MANDATORY)

Before executing ANY SDD command (`/ecomono-sdd-new`, `/ecomono-sdd-ff`, `/ecomono-sdd-continue`, `/ecomono-sdd-explore`, `/ecomono-sdd-status`, `/ecomono-sdd-apply`, `/ecomono-sdd-verify`, `/ecomono-sdd-archive`), check if `ecomono-sdd-init` has been run for this project:

1. Search ecomono-memory: `mem_search(query: "ecomono-sdd-init/{project}", project: "{project}")`
2. If found → init was done, proceed normally
3. If NOT found → run `ecomono-sdd-init` FIRST (delegate to ecomono-sdd-init sub-agent), THEN proceed with the requested command
...
When the user invokes `/ecomono-sdd-new`, `/ecomono-sdd-ff`, or `/ecomono-sdd-continue` (or an equivalent natural-language request, e.g. "haceme un SDD para X" / "do SDD for X") for the first time in a session, ASK which execution mode they prefer:
...
- **Artifact existence:** the declared artifact actually exists and is readable in the active backend — read it back (ecomono-memory: `mem_search` + `mem_get_observation` on the topic key; openspec: read the file path). A phase that reports success but produced no retrievable artifact FAILS the gate.
...
- **Inline for low-risk phases** (`ecomono-sdd-explore`, `ecomono-sdd-spec`, `ecomono-sdd-tasks`, `ecomono-sdd-archive`): the orchestrator runs the checks itself by reading the artifact back. No extra sub-agent.
- **Fresh-context reviewer for high-risk phases** (`ecomono-sdd-design`, `ecomono-sdd-apply`): delegate a fresh-context reviewer sub-agent for independent judgment, because errors in these phases compound downstream. Use the `ecomono-sdd-verify` model alias for the delegated gate review and include `model` per the mandatory model gate.
...
The gatekeeper runs in addition to the Review Workload Guard and the Mandatory Delegation Triggers; it never relaxes them and never auto-marks anything reviewed in ecomono-memory.

### Artifact Store Mode

When the user invokes `/ecomono-sdd-new`, `/ecomono-sdd-ff`, or `/ecomono-sdd-continue` (or an equivalent natural-language request) for the first time in a session, ALSO ASK which artifact store they want for this change:

- **`ecomono-memory`**: Fast, no files created. Artifacts live in ecomono-memory only. Best for solo work and quick iteration. Note: re-running a phase overwrites the previous version (no history).
- **`openspec`**: File-based. Creates `openspec/` directory with full artifact trail. Committable, shareable with team, full git history.
- **`hybrid`**: Both — files for team sharing + ecomono-memory for cross-session recovery. Higher token cost.

If the user doesn't specify, detect: if ecomono-memory is available → default to `ecomono-memory`. Otherwise → `none`.

Cache the artifact store choice for the session. Pass it as `artifact_store.mode` to every sub-agent launch.

### Delivery Strategy

On the first `/ecomono-sdd-new`, `/ecomono-sdd-ff`, or `/ecomono-sdd-continue` (or an equivalent natural-language request) in a session, ask once for and cache delivery strategy: `ask-on-risk` (default), `auto-chain`, `single-pr`, or `exception-ok`. Pass it as `delivery_strategy` to `ecomono-sdd-tasks` and `ecomono-sdd-apply` prompts.
...
Cache the chain strategy for the session. Pass it as `chain_strategy` to `ecomono-sdd-tasks` and `ecomono-sdd-apply` prompts alongside `delivery_strategy`. Do not ask again unless the user changes scope.
...
After `ecomono-sdd-tasks` completes and before launching `ecomono-sdd-apply`, inspect `Review Workload Forecast`.
...
- **`auto-chain`**: Do not ask about splitting. If `chain_strategy` is not yet cached, ask which chain strategy to use. Then pass to `ecomono-sdd-apply`: implement only the next autonomous slice using work-unit commits, with clear start, finish, verification, and rollback boundary.
- **`single-pr`**: STOP and require/record `size:exception` before apply.
- **`exception-ok`**: Continue, but tell `ecomono-sdd-apply` this run uses `size:exception`.

Automatic mode does not override this guard. Always pass the resolved delivery strategy to `ecomono-sdd-apply`.
...
|-------|---------------|--------|--------|
| ecomono-sdd-explore | sonnet | default | Reads code, structural - not architectural |
| ecomono-sdd-propose | opus | default | Architectural decisions |
| ecomono-sdd-spec | sonnet | default | Structured writing |
| ecomono-sdd-design | opus | default | Architecture decisions |
| ecomono-sdd-tasks | sonnet | default | Mechanical breakdown |
| ecomono-sdd-apply | sonnet | default | Implementation |
| ecomono-sdd-verify | sonnet | default | Validation against spec |
| ecomono-sdd-archive | haiku | default | Copy and close |
| ecomono-sdd-onboard | haiku | default | Guided walkthrough, pedagogical |
| ecomono-judge-a | sonnet | default | Adversarial review — blind judge A |
| ecomono-judge-b | sonnet | default | Adversarial review — blind judge B |
...
ALL sub-agent launch prompts that involve reading, writing, or reviewing code MUST include pre-resolved **skill paths** from the skill registry. Follow the **Skill Resolver Protocol** (`~/.claude/skills/ecomono-sdd-shared/skill-resolver.md`).

**Pre-flight before every Agent call (mandatory):**

1. Identify the phase key (`ecomono-sdd-apply`, `ecomono-sdd-verify`, etc.) or use `default` for general delegation.
...
2. Fallback: read `.atl/skill-registry.md` if ecomono-memory not available
...
- Read context: orchestrator searches ecomono-memory (`mem_search`) for relevant prior context and passes it in the sub-agent prompt. Sub-agent does NOT search ecomono-memory itself.
- Write context: sub-agent MUST save significant discoveries, decisions, or bug fixes to ecomono-memory via `mem_save` before returning. Sub-agent has full detail — save before returning, not after.
- Always add to sub-agent prompt: `"If you make important discoveries, decisions, or fix bugs, save them to ecomono-memory via mem_save with project: '{project}'."`
...
| Phase         | Reads                                                  | Writes           |
| ------------- | ------------------------------------------------------ | ---------------- |
| `ecomono-sdd-explore` | nothing                                                | `explore`        |
| `ecomono-sdd-propose` | exploration (optional)                                 | `proposal`       |
| `ecomono-sdd-spec`    | proposal (required)                                    | `spec`           |
| `ecomono-sdd-design`  | proposal (required)                                    | `design`         |
| `ecomono-sdd-tasks`   | spec + design (required)                               | `tasks`          |
| `ecomono-sdd-apply`   | tasks + spec + design + **apply-progress (if exists)** | `apply-progress` |
| `ecomono-sdd-verify`  | spec + tasks + **apply-progress**                      | `verify-report`  |
| `ecomono-sdd-archive` | all artifacts                                          | `archive-report` |

For phases with required dependencies, sub-agent reads directly from the backend — orchestrator passes artifact references (topic keys or file paths), NOT content itself.

#### Strict TDD Forwarding (MANDATORY)

When launching `ecomono-sdd-apply` or `ecomono-sdd-verify` sub-agents, the orchestrator MUST:

1. Search for testing capabilities: `mem_search(query: "ecomono-sdd-init/{project}", project: "{project}")`
...
When launching `ecomono-sdd-apply` for a continuation batch (not the first batch):
...
#### ecomono-memory Topic Key Format

| Artifact        | Topic Key                          |
| --------------- | ---------------------------------- |
| Project context | `ecomono-sdd-init/{project}`               |
...
Convention files under the agent's global skills directory (global) or `.agent/agent-skills/ecomono-sdd-shared/` (workspace): `memory-convention.md`, `persistence-contract.md`, `openspec-convention.md`.
