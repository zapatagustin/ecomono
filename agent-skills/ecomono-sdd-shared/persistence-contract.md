...
The orchestrator ASKs the user which mode they want when `/ecomono-sdd-new`, `/ecomono-sdd-ff`, or `/ecomono-sdd-continue` is invoked for the first time in a session. The choice is cached for the session.
...
To generate/update: run `gentle-ai skill-registry refresh`, or run `ecomono-sdd-init`.
