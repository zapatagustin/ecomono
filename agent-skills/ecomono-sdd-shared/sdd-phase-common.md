...
- The orchestrator MUST pass `delivery_strategy` to `ecomono-sdd-tasks` and the resolved decision to `ecomono-sdd-apply`.
- `ecomono-sdd-tasks` MUST forecast whether the planned work may exceed that budget.
...
- If the forecast is high, `ecomono-sdd-tasks` MUST recommend chained or stacked PRs using deliverable work units.
- `ecomono-sdd-apply` MUST NOT start oversized work unless the delivery strategy resolves to chained/stacked PR slices or explicitly accepted `size:exception`.
