...
> the dedicated `ecomono-sdd-propose` sub-agent using your platform's delegation primitive
...
- Exploration analysis (from ecomono-sdd-explore) OR direct user description
...
- **ecomono-memory**: Read `sdd/{change-name}/explore` (optional) and `ecomono-sdd-init/{project}` (optional). Save artifact as `sdd/{change-name}/proposal`.
...
> The ecomono-sdd-spec agent reads this to know exactly which spec files to create or update.
...
### Next Step
Ready for specs (ecomono-sdd-spec) or design (ecomono-sdd-design).
```
...
- **ALWAYS fill in the Capabilities section** — this is the contract with ecomono-sdd-spec. Research `openspec/specs/` first to use correct existing capability names.
