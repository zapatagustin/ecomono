...
were inert, and have since been rebranded to `<!-- ecomono:* -->` so a manual sync has no
anchor to rewrite. The markers still matter for `claude/CLAUDE.md` and `opencode/AGENTS.md`, which a
...
Deduplicating is not available — the persona was originally generated by the `gentle-ai` binary, which has no
