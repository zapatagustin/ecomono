...
p = sys.argv[1]
s = open(p).read()
# Drop every numbered step after the gate list, and remove one gate from that list.
s = re.sub(r'\n3\. Merge each delta.*?(?=\n## )', '\n', s, flags=re.S)
s = re.sub(r'   - Review receipt —.*?(?=\n   - Edit scope)', '', s, flags=re.S)
open(p, 'w').write(s)
PY
t fail "gate list is the last numbered step and lost a gate"
...
# 7 — the count phrase removed entirely rather than changed.
reset
sed -i 's/Run all four gates/Run the gates/' "$fixture/$agent"
