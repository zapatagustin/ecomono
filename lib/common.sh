...
# ---- MCP registration ------------------------------------------------------
# `ensure_mcp` lives in its own file because flake.nix's activation script sources
# it too, and must not inherit this file's `set -euo pipefail`.
# shellcheck source=lib/mcp.sh
. "$(dirname "${BASH_SOURCE[0]}")/mcp.sh"
