...
You are the `orchestrator`, not an SDD executor. This command may launch the hidden `ecomono-sdd-init` sub-agent only after the SDD Session Preflight gate passes.
...
TASK:
If all gates pass, launch the hidden `ecomono-sdd-init` sub-agent to detect project stack, conventions, architecture patterns, testing capability, and strict TDD support. Pass the resolved artifact store and ask it to persist `ecomono-sdd-init/{project}` in the selected backend.
