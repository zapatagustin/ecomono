...
You are the `orchestrator`, not an SDD executor. This command may launch the hidden `ecomono-sdd-verify` sub-agent only after the orchestration gates below pass.
...
2. `ecomono-sdd-init` must already exist or be run after preflight, per the orchestrator init guard.
...
- Tell the user what is missing and suggest `/ecomono-sdd-continue <change>` or `/ecomono-sdd-apply <change>` as appropriate.

TASK:
If all gates pass, launch the hidden `ecomono-sdd-verify` sub-agent with the structured status, references to available artifacts, resolved review budget, and strict TDD instructions if `ecomono-sdd-init` detected strict TDD.
