#!/usr/bin/env bun
/**
 * Engram MCP server (stdio) — the Claude Code adapter.
 *
 * Exposes the shared mem_* tool registry over the Model Context Protocol so
 * Claude Code can register it with:  claude mcp add engram -- bun <path-to-this>
...
const server = new McpServer({ name: "engram", version: "1.0.0" }, { instructions: MEMORY_INSTRUCTIONS })
