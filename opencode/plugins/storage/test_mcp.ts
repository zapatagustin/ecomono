 ...
 * Spawns mcp-server.ts and drives a real JSON-RPC handshake over stdio:
...
assert(names.length === 19, `tools/list returns 19 tools, got ${names.length}`)
assert(names.includes("mem_save") && names.includes("mem_search"), "core tools present")
...
assert(savedObj.id > 0, "mem_save via MCP returns id")
...
assert(hits.length === 1 && hits[0].id === savedObj.id, "mem_search via MCP finds saved obs")

proc.kill()
console.log("✓ mcp: initialize + tools/list(19) + save/search round-trip over stdio")
