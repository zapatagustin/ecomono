...
- At the end, include: **Skill Resolution**: {paths-injected|fallback-registry|fallback-path|none} — {details}. Report `paths-injected` whenever the prompt named exact files to review against, wherever the delegator found them; `fallback-registry` if you loaded the registry yourself; `none` only if you were handed nothing and had to invent your own bar. The value describes what the delegator gave you, not how well you worked.
